...
use crate::{
    ...
    config::PAGE_SIZE, 
    mm::{copy_to_virt_addr, get_frame_space, MapPermission, PageTable, VPNRange, VirtAddr}, task::{
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    if let Some(app_inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
        let all_data = app_inode.read_all();
        let app_data = all_data.as_slice();
        let task_control_block = Arc::new(TaskControlBlock::new(app_data));
        let task = current_task().unwrap();
        let mut inner = task.inner_exclusive_access();
