...
use crate::{
    config::PAGE_SIZE, loader::get_app_data_by_name, mm::{copy_to_virt_addr, get_frame_space, translated_refmut, translated_str, MapPermission, PageTable, VPNRange, VirtAddr}, task::{
...
pub fn sys_mmap(start: usize, len: usize, prot: usize) -> isize {
    ...
    let mut inner = task.inner_exclusive_access();
    for i in vpn_range {
        inner.memory_set.push_one(i, flags);
    }
    0
}
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    let mut inner = task.inner_exclusive_access();
    for i in vpn_range {
        if !inner.memory_set.has_one(i) {
            return -1;
        }
    }
    for i in vpn_range {
        inner.memory_set.pop_one(i);
    }
    0
}
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    let task_control_block = Arc::new(TaskControlBlock::new(app_data.unwrap()));
    let task = current_task().unwrap();
    let mut inner = task.inner_exclusive_access();
    let pid = task_control_block.pid.0;
    add_task(task_control_block.clone());
    inner.children.push(task_control_block);
    pid as isize
}
